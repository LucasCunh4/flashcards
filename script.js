...
function dbBulkAdd(items){return new Promise((res,rej)=>{const t=db.transaction(STORE,'readwrite'),s=t.objectStore(STORE);items.forEach(c=>{const x={...c};delete x.id;s.add(x)});t.oncomplete=()=>res();t.onerror=()=>rej(t.error)})}
...
const now=()=>Date.now(),isDue=c=>!c.nextReview||c.nextReview<=now(),sameDay=(a,b)=>new Date(a).toDateString()===new Date(b).toDateString(),svgDataUrl=svg=>'data:image/svg+xml;charset=utf-8,'+encodeURIComponent(svg),cardMedia=c=>c.svg?svgDataUrl(c.svg):(c.image||null);
function fmtDate(ts){if(!ts||ts<=now())return'Agora';const d=Math.ceil((ts-now())/DAY);if(d===1)return'Amanhã';if(d<7)return`Em ${d} dias`;return new Date(ts).toLocaleDateString('pt-BR',{day:'2-digit',month:'short'})}
...
els.selectModeBtn.onclick=()=>{if(!cards.length)return toast('Não há flashcards para selecionar.');setSelectionMode(!selectionMode)};els.cancelSelectionBtn.onclick=()=>setSelectionMode(false);
...
function updatePrPreview(){const raw=els.prInput.value,{items,errors}=parsePR(raw),count=items.length;parsedPrItems=items;els.prPreview.innerHTML='';els.prStatus.querySelector('strong').textContent=count;els.prStatus.querySelector('span').textContent=count===1?'flashcard identificado':'flashcards identificados';els.prStatus.className='pr-status '+(!raw.trim()?'neutral':errors.length||!count?'invalid':'valid');if(!raw.trim()){els.prError.classList.add('hidden');els.prPreviewSection.classList.add('hidden');els.savePrBtn.disabled=true;return}if(errors.length){els.prError.classList.remove('hidden');els.prError.textContent=errors.slice(0,3).join(' · ');els.savePrBtn.disabled=true}else{els.prError.classList.toggle('hidden',!!count);if(!count)els.prError.textContent='Não encontrei blocos P: e R:.';els.savePrBtn.disabled=!count}els.prPreviewSection.classList.toggle('hidden',!count);if(!count)return;els.prPreviewInfo.textContent=`${count} ${count===1?'flashcard':'flashcards'}`;items.slice(0,30).forEach((x,i)=>{const card=document.createElement('article'),n=document.createElement('div'),content=document.createElement('div'),q=document.createElement('div'),r=document.createElement('div');card.className='pr-preview-card';n.className='pr-preview-number';n.textContent=i+1;q.className='pr-preview-q';q.textContent=x.question;r.className='pr-preview-r';r.textContent='R: '+x.answer;content.append(q,r);card.append(n,content);els.prPreview.appendChild(card)})}
...
function openEditor(id=null){closePrImporter();if(selectionMode)setSelectionMode(false);els.editor.classList.remove('hidden');els.form.reset();els.editId.value='';editingImage=null;editingSvg=null;bulkEntries=[];setBulkMode(false);setSingleMediaMode('image');updateImagePreview();updateSvgPreview();els.bulkToggle.classList.remove('hidden');if(id){const c=cards.find(x=>x.id===id);if(!c)return;els.editorTitle.textContent='Editar flashcard';els.bulkToggle.classList.add('hidden');els.editId.value=c.id;els.question.value=c.question;els.answer.value=c.answer;if(c.svg){editingSvg=c.svg;els.svgInput.value=c.svg;setSingleMediaMode('svg');updateSvgPreview()}else{editingImage=c.image||null;updateImagePreview()}}else els.editorTitle.textContent='Novo flashcard';els.editor.scrollIntoView({behavior:'smooth'})}
function closeEditor(){els.editor.classList.add('hidden');els.form.reset();els.editId.value='';editingImage=null;editingSvg=null;bulkEntries=[];setBulkMode(false);setSingleMediaMode('image');updateImagePreview();updateSvgPreview()}
function updateImagePreview(){const has=!!editingImage;els.imagePreviewWrap.classList.toggle('hidden',!has);els.removeImage.classList.toggle('hidden',!has);if(has)els.imagePreview.src=editingImage;else els.imagePreview.removeAttribute('src')}
function updateSvgPreview(){const has=!!editingSvg;els.svgPreviewWrap.classList.toggle('hidden',!has);if(has)els.svgPreview.src=svgDataUrl(editingSvg);else els.svgPreview.removeAttribute('src')}
async function imageToDataURL(file,max=1600,quality=.82){const url=URL.createObjectURL(file);try{const img=await new Promise((res,rej)=>{const im=new Image;im.onload=()=>res(im);im.onerror=rej;im.src=url});let{width,height}=img;if(width>max||height>max){const s=Math.min(max/width,max/height);width=Math.round(width*s);height=Math.round(height*s)}const canvas=document.createElement('canvas');canvas.width=width;canvas.height=height;canvas.getContext('2d').drawImage(img,0,0,width,height);return canvas.toDataURL('image/jpeg',quality)}finally{URL.revokeObjectURL(url)}}
els.previewSvgBtn.onclick=()=>{try{editingSvg=sanitizeSvg(els.svgInput.value);els.svgInput.value=editingSvg;updateSvgPreview();toast('SVG válido.')}catch(e){editingSvg=null;updateSvgPreview();toast('SVG inválido.')}};els.clearSvgBtn.onclick=()=>{editingSvg=null;els.svgInput.value='';updateSvgPreview()};

function renderBulkEntries(){els.bulkItems.innerHTML='';els.bulkCount.textContent=bulkEntries.length?`${bulkEntries.length} ${bulkEntries.length===1?'item adicionado':'itens adicionados'}`:'Nenhum item adicionado';bulkEntries.forEach((entry,index)=>{const item=document.createElement('article'),media=document.createElement('div'),img=document.createElement('img'),type=document.createElement('span'),body=document.createElement('div'),head=document.createElement('div'),title=document.createElement('strong'),remove=document.createElement('button'),ta=document.createElement('textarea');item.className='bulk-item';media.className='bulk-media';img.alt='';img.src=entry.type==='svg'?svgDataUrl(entry.svg):entry.image;type.className='bulk-type';type.textContent=entry.type==='svg'?'SVG':'FOTO';media.append(img,type);head.className='bulk-item-head';title.textContent=`Flashcard ${index+1}`;remove.className='small-btn delete';remove.type='button';remove.textContent='×';remove.onclick=()=>{bulkEntries.splice(index,1);renderBulkEntries()};head.append(title,remove);ta.rows=3;ta.placeholder='Resposta...';ta.value=entry.answer||'';ta.oninput=()=>bulkEntries[index].answer=ta.value;body.append(head,ta);item.append(media,body);els.bulkItems.appendChild(item)})}
els.bulkToggle.onclick=()=>{if(!els.editId.value)setBulkMode(!bulkMode)};
els.bulkImageInput.onchange=async()=>{const files=[...els.bulkImageInput.files];els.bulkImageInput.value='';if(!files.length)return;try{toast(`Preparando ${files.length} ${files.length===1?'imagem':'imagens'}...`);for(const file of files){if(!file.type.startsWith('image/'))continue;bulkEntries.push({type:'image',image:await imageToDataURL(file),svg:null,answer:''})}renderBulkEntries();toast('Imagens adicionadas.')}catch(e){console.error(e);toast('Não consegui processar uma das imagens.')}};
els.openBulkSvgBtn.onclick=()=>{els.bulkSvgBox.classList.remove('hidden');setTimeout(()=>els.bulkSvgInput.focus(),100)};els.cancelBulkSvgBtn.onclick=()=>els.bulkSvgBox.classList.add('hidden');
els.addBulkSvgBtn.onclick=()=>{try{const svgs=extractSvgs(els.bulkSvgInput.value);if(!svgs.length)return toast('Não encontrei nenhum SVG.');svgs.forEach(svg=>bulkEntries.push({type:'svg',svg,image:null,answer:''}));els.bulkSvgInput.value='';els.bulkSvgBox.classList.add('hidden');renderBulkEntries();toast(`${svgs.length} ${svgs.length===1?'SVG adicionado':'SVGs adicionados'}.`)}catch(e){console.error(e);toast('SVG inválido.')}};

els.form.onsubmit=async e=>{e.preventDefault();const question=els.question.value.trim(),id=Number(els.editId.value);if(!question)return toast('Preencha a pergunta.');try{if(bulkMode&&!id){if(!bulkEntries.length)return toast('Adicione imagens ou SVGs.');if(bulkEntries.some(x=>!x.answer.trim()))return toast('Preencha todas as respostas.');const t=now();await dbBulkAdd(bulkEntries.map(x=>({question,answer:x.answer.trim(),image:x.type==='image'?x.image:null,svg:x.type==='svg'?x.svg:null,favorite:false,createdAt:t,updatedAt:t,reviews:0,lapses:0,ease:2.3,interval:0,nextReview:0,lastReviewed:null})));toast(`${bulkEntries.length} ${bulkEntries.length===1?'flashcard criado':'flashcards criados'}.`)}else{const answer=els.answer.value.trim();if(!answer)return toast('Preencha a resposta.');let image=null,svg=null;if(singleMediaMode==='image')image=editingImage;else if(els.svgInput.value.trim())svg=sanitizeSvg(els.svgInput.value);if(id){const old=cards.find(c=>c.id===id);await dbPut({...old,question,answer,image,svg,updatedAt:now()});toast('Flashcard atualizado.')}else{await dbAdd({question,answer,image,svg,favorite:false,createdAt:now(),updatedAt:now(),reviews:0,lapses:0,ease:2.3,interval:0,nextReview:0,lastReviewed:null});toast('Flashcard criado.')}}closeEditor();await loadCards()}catch(e){console.error(e);toast('Não foi possível salvar.')}};
