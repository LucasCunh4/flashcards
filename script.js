...
let db,cards=[],editingImage=null,editingSvg=null,singleMediaMode='image',
bulkMode=false,bulkEntries=[],parsedPrItems=[],prParseTimer,
...
const cardMedia=c=>
  c.svg?svgDataUrl(c.svg):(c.image||null);
...
function setSelectionMode(on){
  ...
  els.selectModeBtn.textContent=on
    ?'✓ Selecionando'
    :'☑ Selecionar';
...
els.selectModeBtn.onclick=()=>{
  if(!cards.length)
    return toast('Não há flashcards para selecionar.');

  setSelectionMode(!selectionMode);
};

els.cancelSelectionBtn.onclick=()=>
...
function parsePR(text){
  ...
  items.forEach((item,index)=>{
    if(!item.question)
      errors.push(`Flashcard ${index+1}: pergunta vazia.`);
    else if(!item.answer)
      errors.push(`Flashcard ${index+1}: resposta vazia.`);
  });
...
function openPrImporter(){
  ...
  els.prImportPanel.scrollIntoView({
    behavior:'smooth',
    block:'start'
  });
...
function updatePrPreview(){
  ...
  els.prStatus.querySelector('span').textContent=
    count===1
      ?'flashcard identificado'
      :'flashcards identificados';

  els.prStatus.className=
    'pr-status '+
    (
      !raw.trim()
        ?'neutral'
  ...
  if(errors.length){
    ...
    if(!count)
      els.prError.textContent=
        'Não encontrei blocos P: e R:.';

    els.savePrBtn.disabled=!count;
  }

  els.prPreviewSection.classList.toggle(
    'hidden',
    !count
  );
  ...
  items.slice(0,30).forEach((item,index)=>{
    ...
    card.innerHTML=
      `<div class="pr-preview-number">${index+1}</div>`;
...
function render(){
  ...
  const today=
    cards.filter(
      c=>
        c.lastReviewed&&
        sameDay(c.lastReviewed,Date.now())
    ).length;
  ...
  /* Agora mostra o TOTAL de cartões que entrarão na sessão */
  els.studyBtn.innerHTML=
    cards.length
      ?`<span>▶</span> Estudar agora <b>${cards.length}</b>`
...
function cardRow(c){
  ...
  if(selectionMode){
    ...
    cb.setAttribute(
      'aria-label',
      'Selecionar flashcard'
    );

    cb.onchange=()=>
      toggleSelected(c.id,cb.checked);
  ...
  $$('[data-media]').forEach(
    btn=>
      btn.classList.toggle(
        'active',
        btn.dataset.media===mode
      )
  );

  els.imageControls.classList.toggle(
    'hidden',
    mode!=='image'
  );

  els.svgControls.classList.toggle(
    'hidden',
    mode!=='svg'
  );
}
...
$$('[data-media]').forEach(
  btn=>
    btn.onclick=()=>
      setSingleMediaMode(btn.dataset.media)
);
...
function setBulkMode(on){
  ...
  els.singleArea.classList.toggle(
    'hidden',
    on
  );
...
function updateSvgPreview(){
  ...
  els.svgPreviewWrap.classList.toggle(
    'hidden',
    !has
  );
  ...
  if(has)
    els.svgPreview.src=
      svgDataUrl(editingSvg);
  else
    els.svgPreview.removeAttribute('src');
}
...
els.previewSvgBtn.onclick=()=>{
  try{
    editingSvg=
      sanitizeSvg(els.svgInput.value);
...
els.form.onsubmit=async e=>{
  ...
  try{
    if(bulkMode&&!id){
      ...
      toast(
        `${bulkEntries.length} flashcards criados.`
      );
    }else{
      const answer=
...
/*
  NOVA DINÂMICA:
  - Todos os cartões entram na sessão.
  - Os vencidos aparecem primeiro.
  - Os demais vêm depois.
  - O modo embaralhado embaralha cada grupo.
*/
function startStudy(){
  if(!cards.length)
    return toast(
      'Não há flashcards para estudar.'
    );
...
function nextStudyCard(){
  ...
  const scroll=
    document.querySelector('.study-scroll');
...
function revealAnswer(){
  ...
  if(
    !els.answerArea.classList.contains(
      'hidden'
    )
  )
    return;
...
async function rateCard(rating){
  ...
  if(rating==='again'){
    ...
    /*
      Se errou, o cartão volta para o fim
      da sessão atual.
    */
...
document.addEventListener(
  ...
  e=>{
    if(
      !els.studyView.classList.contains(
        'active'
      )
    )
      return;
    ...
    if(
      !els.ratingArea.classList.contains(
        'hidden'
      )&&
